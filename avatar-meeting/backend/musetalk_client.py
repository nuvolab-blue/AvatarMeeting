...
No retries, no log spam. Local lip-sync handles everything client-side.
"""
...
class MuseTalkClient:
    ...
    def __init__(self, base_url: str = _DEFAULT_URL) -> None:
        ...
        self._available = False  # only True after successful health check
        self._checked = False
    ...
    async def health_check(self) -> bool:
        ...
        if not self._available:
            logger.info("MuseTalk not available — local lip-sync will be used")
        return self._available

    async def prepare(self, image_path: str, bbox_shift: int = 5) -> dict:
        """Send image for preprocessing. Skips silently if unavailable."""
        if not self._available:
